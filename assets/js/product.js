...
// let currentSort = ''; // lưu kiểu sắp xếp hiện tại

// function shuffer(sortType = currentSort) {
// currentSort = sortType; // ghi nhớ kiểu lọc chọn hiện tại

// fetch(dataUrl)
//     .then(r => r.json())
//     .then(list => {
//     // Sắp xếp nếu có sortType
//     if (sortType === 'price-asc') {
//         list.sort((a, b) => Number(a.newPrice.replace(/\./g, '')) - Number(b.newPrice.replace(/\./g, '')));
//     } else if (sortType === 'price-desc') {
//         list.sort((a, b) => Number(b.newPrice.replace(/\./g, '')) - Number(a.newPrice.replace(/\./g, '')));
//     } else if (sortType === 'best-selling') {
//         const parseSaled = s => parseFloat(s.replace(',', '.')) * 1000;
//         list.sort((a, b) => parseSaled(b.saled) - parseSaled(a.saled));
//     } else {
//         // nếu không truyền type — giữ nguyên hoặc random
...
function renderItem(items) {
    ...
    var htmls = shuffledItems.map(function (item) {
        return `
...
function shuffer() {
    renderItem(products); // `products` là mảng sản phẩm gốc của bạn
    responsive();
}
...
function filterByCity() {
    const checkedBoxes = document.querySelectorAll('.category-group-item-check:checked');
    const selectedCities = Array.from(checkedBoxes).map(cb => cb.value);
    ...
    const priceInputs = document.querySelectorAll('.category-group-filter-input');
    const minPrice = parseInt(priceInputs[0].value.replace(/\D/g, '')) || 0;
    const maxPrice = parseInt(priceInputs[1].value.replace(/\D/g, '')) || Infinity;
